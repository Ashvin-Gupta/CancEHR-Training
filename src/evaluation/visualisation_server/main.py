            ...
            if config_path.exists():
                try:
                    ...
                    try:
                        ...
                        # Debug logging
                        print(f"Experiment {experiment_dir.name}: train_path={train_path}, val_path={val_path}, dataset_name={dataset_name}")
...
def analyze_simulation_data(csv_path, experiment_name):
    ...
    try:
        ...
        for start_idx in start_indices:
            ...
            stop_reason_percentages[start_idx] = {
                'max_steps_pct': (stop_reason_by_start[start_idx]['max_steps'] / total) * 100 if total > 0 else 0,
                'stop_token_pct': (stop_reason_by_start[start_idx]['stop_token'] / total) * 100 if total > 0 else 0
            }
...
def get_experiments_with_models():
...
async def losses(request: Request):
    ...
    return templates.TemplateResponse("losses.html", {
        "request": request,
        "loss_data": loss_data
    })
...
async def simulations(request: Request):
    ...
    return templates.TemplateResponse("simulations.html", {
        "request": request,
        "simulation_experiments": simulation_experiments
    })
...
async def inference(request: InferenceRequest):
    ...
    try:
        ...
        return {
            ...
            "input_length": len(request.input_tokens)
        }
