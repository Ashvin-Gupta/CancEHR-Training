...
def simulate_subject(
    ...
    Results are saved to a CSV file in the save_dir.
    ...
    model.load_state_dict(
        torch.load(os.path.join(experiment_dir, "model.pth"), map_location=device)
    )
    model.to(device)
    model.eval()
...
def simulate_batch(
    ...
    Args:
        input_tokens (torch.Tensor): The input tokens for the batch.
        start_token_index (int): The index of the start token.
        simulation_start_idx (int): The index of the simulation start.
        batch_size (int): The number of simulations to run in parallel.
    ...
    with torch.no_grad():
        for step in tqdm(range(max_steps), desc="Steps", leave=False):
            ...
            padded_contexts = torch.zeros(
                len(active_indices), max_context_len, dtype=torch.long, device=device
            )
    ...
    for i in range(batch_size):
        ...
        predicted_tokens = (
            batch_sequences[i, len(input_tokens) : sequence_lengths[i]].cpu().tolist()
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--device", type=str, default="cuda", choices=["cpu", "cuda"], help="Device to run on"
    )
    parser.add_argument(
        "--batch_size", type=int, default=10, help="Number of simulations to batch together"
    )
    parser.add_argument(
    ...
    stop_tokens_indices = [
        eval_dataset.string_to_token(stop_token_str) for stop_token_str in stop_tokens_str
    ]
