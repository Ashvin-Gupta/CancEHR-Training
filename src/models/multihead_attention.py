...
class MultiHeadAttention(torch.nn.Module):
    ...
    """
    def __init__(self, d_input, d_output, n_heads, dropout):
        super().__init__()
        self.heads = torch.nn.ModuleList([AttentionHead(d_input, d_output, dropout) for _ in range(n_heads)])
...
class AttentionHead(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        q = self.q(x) # (batch_size, seq_len, d_input) @ (d_input, d_output) = (batch_size, seq_len, d_output)
        k = self.k(x) # (batch_size, seq_len, d_input) @ (d_input, d_output) = (batch_size, seq_len, d_output)

        # calculate raw attention
        k_T = k.transpose(-2, -1) # (batch_size, seq_len, d_output) -> (batch_size, d_output, seq_len)
        raw_attn = torch.matmul(q, k_T) # (batch_size, seq_len, d_output) @ (batch_size, d_output, seq_len) = (batch_size, seq_len, seq_len)
        ...
        v = self.v(x) # (batch_size, seq_len, d_input) @ (d_input, d_output) = (batch_size, seq_len, d_output)
        output = torch.matmul(attn, v) # (batch_size, seq_len, seq_len) @ (batch_size, seq_len, d_output) = (batch_size, seq_len, d_output)
